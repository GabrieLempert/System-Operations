...
int main(int argc, char **argv)
{
    ...
    for (generalNum = lval; (generalNum <= uval) && (nval != 0); generalNum++)
        if (flagarr[generalNum - lval])
        {
            count--;
            nval--;
            printf("%d%c", generalNum, nval ? ',' : '\n');
        }
